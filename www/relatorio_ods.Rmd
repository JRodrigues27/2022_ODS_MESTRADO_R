...
![](logo_mestrado.png)
---
# RELATÓRIO SAÚDE AMBIENTAL - 0DS `r params$acoes`

`r format(Sys.time(), '%d/%m/%y')`
---

## RELATÓRIO ODS PAVS `r params$acoes`^[**ATENÇÃO:** Evite impressões desnecessárias. Se for mesmo preciso imprimir esse documento, prefira a opção em formato livreto, frente e verso ou rascunho. Menos é mais.]
...
```
 ...
 @Corvalan2016, [@Andrade2020, @Palmierei2022]

Este relatório é uma versão imprimível de **`r params$acoes`**, extraído do painel de correlação entre as estratégias de saúde ambiental da atenção primária do município de São Paulo e os Objetivos do Desenvolvimento Sustentável. A integra do painel pode ser acessado on-line clicando no link Painel Interativo Saúde Ambiental - ODS Santa Marcelina ^[https://jrodrigues.shinyapps.io/saudeods/]. 
Neste relatório estáo relacionadas as ações `r params$acoes` que é uma das possibilidades de intervenções que as equipes da Unidades Básicas de Saúde realizam através do Programa Ambientes Verdes e Saudáveis (PAVS). de Essas ações são possibilidade de linhas de atuação do Programa Ambientes Verdes e Saudáveiscomposto segundo as informações registradas no *Monitoramento PAVS* pelo Agente de Promoção Ambiental da  no período:
...
bd_agravos %>% 
  ...
  # )+
  labs(title = stringr::str_wrap(params$acoes, 50),
       subtitle = "agravos, doenças e fração atribuível aos riscos ambientais - FAA",
